...
import TableOfContents, { NotePage } from './TableOfContents';
          ...
          {currentView === 'cover' ? (
            ...
            >
              <TableOfContents
                ...
                pages={pages}
                onPageClick={(index: number) => onNavigate('page', index)}
                onAddPage={onAddPage}
                theme={theme}
                readOnly={false}
              />
              ...
              <div className="flex-1 overflow-y-auto p-6 space-y-3">
                {pages.map((p, i) => (
                  <button
                    ...
                    className={`w-full flex items-center justify-between p-4 rounded-[1.5rem] transition-all border-2 ${currentPageIndex === i
...
function ToolbarBtn({ icon: Icon, onClick, active }: { icon: any, onClick: () => void, active?: boolean }) {
  return (
    <button
      onClick={onClick}
      className={`w-10 h-10 shrink-0 flex items-center justify-center rounded-[1rem] transition-all active:scale-90 ${active
...
function NavBtn({ icon: Icon, active, onClick }: { icon: any, active: boolean, onClick: () => void }) {
  return (
    <button
      onClick={onClick}
      className={`w-[3.25rem] h-[3.25rem] flex items-center justify-center rounded-full transition-all ${active ? 'bg-surface text-foreground shadow-lg scale-100' : 'text-surface/40 hover:text-surface scale-90 hover:scale-100'
        }`}
    >
